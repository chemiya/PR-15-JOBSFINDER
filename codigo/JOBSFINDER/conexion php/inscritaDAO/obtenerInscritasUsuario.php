...
if(isset($data->idUsuario) 
	&& !empty(trim($data->idUsuario))
		...
		$allOfertas = mysqli_query($db_conn,"SELECT o.idOferta as idOferta, o.descripcion as descripcion, o.estado as estado, u.username as username FROM inscripcion i, oferta o, usuario u  where i.idUsuario='$idUsuario' and i.idOferta=o.idOferta and o.idUsuario=u.idUsuario");//hace consulta para buscar uno
		...
		if(mysqli_num_rows($allOfertas) > 0){//si obteiene mas de 1
			...
			while($row = mysqli_fetch_array($allOfertas)){//coge uno del array
				...
				$json_array["userdata"][] = $viewjson;//junta todos
				  ...
				  echo json_encode(["success"=>true,"inscritas"=>$json_array]);//lo devuelve
				  return;
