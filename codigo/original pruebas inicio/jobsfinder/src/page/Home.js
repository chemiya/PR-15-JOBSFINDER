...
const Home = () => {
  ...
  const alluser = async (ids) => {
    try {
      axios.get(`http://localhost/php/JOBSFINDERpruebas/users.php`)
  ...
  const deleteUser = async (id) => {
    try {
      axios.post(`http://localhost/php/JOBSFINDERpruebas/deleteusers.php`, { //le digo a la api que lo borre y le paso el id
        userids: id,
      })
      .then(res => {
        setuser([]);
